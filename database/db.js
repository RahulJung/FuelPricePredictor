...
// Query for Fuel form
const postFuelForm = (gallon, address, date, callback) => {
  connection.query(
    `INSERT INTO register (username, pwd) VALUES ('${gallon}', '${address}' '${date}')`,
    (err, data) => {
      if (err) {
        console.log("problem posting fuel form in query");
        callback(err, null);
      } else {
        callback(null, data);
      }
...
module.exports = { getUserInfo, registerUser, postFuelForm };
